...
public class CursorController : MonoBehaviour
{
    ...
    void Start()
    {
        currentPearl = Instantiate(pearl1, new Vector3(transform.position.x + Random.Range(-0.01f, 0.01f), transform.position.y, transform.position.z), Quaternion.Euler(new Vector3(0, 0, Random.Range(0, 360))));
    ...
    void Update()
    {
        ...
        if (placeDelay <= 0 && holdingPearl == false)
        {
            ...
            currentPearl = Instantiate(pearlToUse, new Vector3(transform.position.x + Random.Range(-0.01f,0.01f), transform.position.y, transform.position.z), Quaternion.Euler(new Vector3(0, 0, Random.Range(0, 360))));
            currentPearl.GetComponent<Rigidbody2D>().bodyType = RigidbodyType2D.Kinematic;
            holdingPearl = true;
        }
    }
